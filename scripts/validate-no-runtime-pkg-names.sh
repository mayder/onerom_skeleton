...
for pattern in "${patterns[@]}"; do
  if grep -RInE "$pattern" "${runtime_dirs[@]}" --include='*.py' --exclude='test_*.py' --exclude='*_test.py' --exclude-dir=.venv --exclude-dir=venv --exclude-dir=dist --exclude-dir=build --exclude-dir=__pycache__; then
